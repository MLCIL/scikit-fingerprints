...
from skfp.fingerprints._new_mordred.descriptors.distance_matrix import (
    FEATURE_NAMES,
    calc,
)
...
./references/distance_matrix.json as expected[molecule][descriptor].
...
def computed_values(mordred_test_mols):
    ...
    for name in _REFERENCE:
        ...
        values = calc(AtomicProperties(mol_regular), n_frags, distance_matrix_regular)
